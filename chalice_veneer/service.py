from chalice import Chalice, IAMAuthorizer, CognitoUserPoolAuthorizer, CORSConfig
...
class Service:
    def __init__(self, app: Chalice, routes: Optional[List[Type[Route]]] = None,
    ...
    def register_service_blueprint(self, service_blueprint: ServiceBlueprint, parent_prefix: Optional[str] = None,
                                   parent_service_blueprint: Optional["ServiceBlueprint"] = None):
        service_blueprint.propagate(
            parent_service_blueprint.authorizer if parent_service_blueprint else self.authorizer,
